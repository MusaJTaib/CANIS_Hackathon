...
data = pd.read_csv('Sentiment_Analysis_TRUE.csv', sep=',', header=0,  names=["index", "idx", "text", "blob_polarity",
...
for i in df_subject["blob_subjectivity"]:
    if 0.0 <= i < 0.1:
...
x_polarity = [i * 0.1 for i in range(11)]
...
x_bar = [i * 0.1 for i in range(11)]
y_bar = [1 for i in range(11)]

# BARkgrounds
for i in range(11):
    plt.bar(x=x_bar[i], height=y_bar[i], color=(1.0 - i*0.09, i*0.1, i*0.05), alpha=0.8, width=0.1)

plt.bar(x=0.0, height=1.0, color=(0.6, 0, 0), alpha=1.0, width=0.02)
...
for a in x_bar:
    plt.text(a, 1.0, "%.1f" % (0.0) if a <= 0.0 else "%.1f~%.1f" % (a, a+0.1) if a < 1.0 else "%.1f" % 1.0,
